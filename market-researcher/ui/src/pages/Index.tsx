...
const Index = () => {
  ...
  const handleGenerateReport = async (researchModel: ResearchModel) => {
    ...
    try {
      ...
      setGenerationError(
        error instanceof Error ? error.message : "Unable to generate the stock digest.",
      );
    } finally {
  ...
  return (
    <div className="app-shell">
      ...
      <div className="research-workspace">
        <Header />
        {showReport ? (
          <DailyDigestReport tickers={tickers} onReset={handleReset} stockDigest={stockDigest} />
        ) : (
          <>
            <main>
              {/* Header */}
              ...
              {/* Main Input Card */}
              <Card className="research-surface portfolio-form">
                ...
                <CardContent className="p-0">
                  ...
                  {isGenerating && (
                    <section
                      className="research-stream-output"
                      aria-labelledby="research-stream-title"
                      aria-live="polite"
                    >
                      ...
                      <ol>
                        {generationEvents.map((event, index) => (
                          <li
                            key={`${index}-${event}`}
                            className={index === generationEvents.length - 1 ? "is-current" : ""}
                          >
                            <span>{event}</span>
                          </li>
                        ))}
        ...
        <footer className="demo-footer">
          <a
            ...
            href="https://docs.tavily.com/documentation/api-reference/endpoint/research-streaming"
            target="_blank"
            rel="noopener noreferrer"
          >
            Research API docs <span aria-hidden="true">↗</span>
          </a>
        </footer>
      </div>
    </div>
