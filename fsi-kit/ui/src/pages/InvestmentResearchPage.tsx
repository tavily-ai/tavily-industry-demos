...
function MeetingBrief({ result }: { result: InvestmentResearchResult }) {
  ...
  <div className="grid md:grid-cols-2 gap-5 mt-6">
    {result.official_policy && <BriefSection title="Official policy"><Evidence items={result.official_policy.developments}/><div className="mt-3"><Evidence items={result.official_policy.official_positions}/></div><div className="mt-3"><Bullets items={result.official_policy.watch_items}/></div></BriefSection>}
    {result.economic_data && <BriefSection title="Economic data"><p className="text-sm text-ink-300 mb-3">{result.economic_data.trend_summary}</p><Evidence items={result.economic_data.indicators}/><div className="mt-3"><Bullets items={result.economic_data.data_limitations}/></div></BriefSection>}
    {result.market_expectations && <BriefSection title="Market expectations"><Evidence items={result.market_expectations.consensus}/><div className="mt-3"><Evidence items={result.market_expectations.disagreements}/></div><div className="mt-3"><Evidence items={result.market_expectations.market_signals}/></div></BriefSection>}
    {result.portfolio_implications && <BriefSection title="Portfolio implications"><Evidence items={result.portfolio_implications.implications}/><div className="mt-3"><Bullets items={result.portfolio_implications.affected_assets_or_sectors}/></div><div className="mt-3"><Bullets items={result.portfolio_implications.transmission_channels}/></div></BriefSection>}
    {result.scenarios && <BriefSection title="Scenarios & counter-thesis"><div className="space-y-2 text-sm text-ink-300"><p><strong>Base:</strong> {result.scenarios.base_case}</p><p><strong>Upside:</strong> {result.scenarios.upside_case}</p><p><strong>Downside:</strong> {result.scenarios.downside_case}</p></div><div className="mt-3"><Evidence items={result.scenarios.counter_thesis}/></div><div className="mt-3"><Bullets items={result.scenarios.signposts}/></div></BriefSection>}
    <BriefSection title="Discussion questions"><Bullets items={result.discussion_questions}/></BriefSection>
    <BriefSection title="Evidence gaps"><Bullets items={result.evidence_gaps}/>{Object.keys(result.lane_errors || {}).length > 0 && <div className="mt-3 rounded-lg bg-risk-high/10 p-3 text-xs text-risk-high">Some workstreams were incomplete: {Object.entries(result.lane_errors).map(([lane,message]) => `${lane}: ${message}`).join(" · ")}</div>}</BriefSection>
  </div></section>;
}
