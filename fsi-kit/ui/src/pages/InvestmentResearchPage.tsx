...
import {
  InvestmentEvidencePoint,
  InvestmentResearchResult,
  WorkflowLaneDefinition,
} from "../types";

const INVESTMENT_LANES: WorkflowLaneDefinition[] = [
  { id: "official_policy", label: "Official & policy" },
  { id: "economic_data", label: "Economic data" },
  { id: "market_expectations", label: "Market expectations" },
  { id: "portfolio_implications", label: "Portfolio implications" },
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item) => (
        <span
          key={item}
          className="rounded-full bg-white/55 border border-white/70 px-2.5 py-1 text-xs text-ink-300"
        >
          {item}
        </span>
      ))}
...
function Scenario({
  ...
  label: string;
  value: string;
  tone: "base" | "upside" | "downside";
}) {
  const toneClass =
    tone === "base"
      ? "border-tavily-lavender"
      : tone === "upside"
        ? "border-tavily-green"
        : "border-risk-elevated";
  return (
    <div className={`border-l-2 ${toneClass} pl-4 py-1`}>
      <p className="text-xs uppercase tracking-wider font-semibold text-ink-500">{label}</p>
      <p className="text-sm text-ink-300 leading-6 mt-1">{value}</p>
    </div>
  );
}
    ...
    <article className="glass rounded-2xl p-5 md:p-8">
      <div className="max-w-4xl mx-auto">
        <header className="pb-7">
          ...
          <h2 className="font-display text-2xl md:text-3xl font-semibold text-ink-100 mt-3 leading-tight">
            {result.topic}
          </h2>
          <div className="flex flex-wrap gap-2 mt-3">
            {[result.meeting_objective, result.audience, result.horizon]
              .filter(Boolean)
              .map((value) => (
                <span
                  key={value}
                  className="text-[11px] text-ink-400 bg-white/50 border border-white/70 rounded-full px-2.5 py-1"
                >
                  {value}
                </span>
              ))}
        ...
        <section className="rounded-xl bg-tavily-orange/[.07] border-l-4 border-tavily-orange px-5 py-4 mb-2">
          ...
          <p className="text-[15px] leading-7 text-ink-200 mt-2 whitespace-pre-line">
        ...
        {result.economic_data && (
          <BriefSection
            number="02"
            title="Economic and fundamental data"
            intro={result.economic_data.trend_summary}
          >
        ...
        {result.scenarios && (
          <BriefSection number="05" title="Scenarios and counter-thesis">
            <div className="space-y-5">
              ...
              <Scenario
                label="Downside case"
                value={result.scenarios.downside_case}
                tone="downside"
              />
            </div>
          ...
          <BriefSection number="07" title="Evidence gaps and caveats">
            ...
            {Object.keys(result.lane_errors || {}).length > 0 && (
              <div className="mt-4 rounded-lg bg-risk-high/10 p-3 text-xs text-risk-high leading-5">
                Some workstreams were incomplete:{" "}
...
export default function InvestmentResearchPage() {
  ...
  const workflow = useWorkflowStream<InvestmentResearchResult>(
    "/api/investment-research/stream",
    lanes,
  );
    ...
    <main>
      <header className="mb-6">
        ...
        <h1 className="font-display text-3xl font-semibold text-ink-100 mt-3">
          Build a meeting-ready brief
        </h1>
        <p className="text-sm text-ink-400 mt-1.5 max-w-3xl">
          Parallel research lanes synthesize policy, data, expectations, implications, and
          counter-theses into a cited brief.
        </p>
      </header>
      ...
      <section className="glass rounded-2xl p-5 mb-5">
        ...
        >
          ...
          <label className="block text-xs font-semibold text-ink-300">
            ...
            <input
              ...
              placeholder="Decide what to monitor"
              disabled={running}
            />
          </label>
          <label className="block text-xs font-semibold text-ink-300">
        ...
        <p className="text-[10px] text-ink-500 leading-relaxed mt-3">
          Public-web research can be incomplete or stale. Validate material claims before making
      ...
      <div className="space-y-5">
        ...
        {workflow.status === "cancelled" && (
          <div className="glass-subtle rounded-xl p-3 text-xs text-ink-400">
            Research stopped. Completed lane evidence remains visible; no final brief was inferred.
          </div>
        )}
        <WorkstreamLanes
          lanes={workflow.lanes}
          heading="Parallel research workstreams"
          variant="list"
        />
