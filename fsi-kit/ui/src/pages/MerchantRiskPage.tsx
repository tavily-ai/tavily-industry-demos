...
import { AlertTriangle, ArrowDown, CheckCircle2, Circle, FileSearch, Fingerprint, Info, Loader2, Play, Square } from "lucide-react";
...
import { MerchantEvidence, MerchantIdentity, MerchantRiskResult, WorkflowLaneDefinition } from "../types";

const MERCHANT_LANES: WorkflowLaneDefinition[] = [
  { id: "identity", label: "Web identity checkpoint" }, { id: "category_fit", label: "Category fit" }, { id: "restricted_products", label: "Restricted products" }, { id: "reputation", label: "Reputation & practices" }, { id: "legal_regulatory", label: "Legal & regulatory" },
];
...
const List=({items,empty}:{items?:string[];empty?:string}) => items?.length ? <ul className="space-y-2.5">{items.map((item,index)=><li className="text-sm text-ink-300 leading-6 flex gap-3" key={index}><span className="w-1.5 h-1.5 rounded-full bg-tavily-orange shrink-0 mt-2"/><span>{item}</span></li>)}</ul> : empty ? <p className="text-xs text-ink-500 italic">{empty}</p> : null;

function EvidenceRows({items}:{items?:MerchantEvidence[]}) {
  if(!items?.length) return null;
  return <div className="divide-y divide-ink-700/35">{items.map((item,index)=><article key={`${item.finding}-${index}`} className="py-4 first:pt-0 last:pb-0 grid grid-cols-[24px_1fr] gap-3">
    <span className="w-6 h-6 rounded-full bg-tavily-orange/10 text-tavily-orange text-[11px] font-semibold flex items-center justify-center mt-0.5">{index+1}</span>
    <div className="min-w-0"><div className="flex items-start justify-between gap-3"><h4 className="text-sm font-semibold text-ink-100 leading-6">{item.finding}</h4>{item.observed_date&&<span className="text-[10px] text-ink-500 whitespace-nowrap mt-1">{item.observed_date}</span>}</div><p className="text-sm text-ink-300 leading-6 mt-1">{item.significance}</p>{item.source_urls.length>0&&<div className="flex flex-wrap gap-x-3 gap-y-1 mt-2">{item.source_urls.map((url,i)=><a key={url} href={url} target="_blank" rel="noreferrer" className="text-[11px] font-medium text-tavily-orange hover:underline">Evidence {i+1} ↗</a>)}</div>}</div>
  </article>)}</div>;
}

function ReportSection({number,title,children}:{number:string;title:string;children:React.ReactNode}) {
  return <section className="border-t border-ink-700/40 py-7"><div className="flex gap-3 items-baseline mb-4"><span className="text-xs font-mono font-semibold text-tavily-orange">{number}</span><h3 className="font-display text-lg font-semibold text-ink-100">{title}</h3></div>{children}</section>;
}

function Subheading({children}:{children:React.ReactNode}) { return <h4 className="text-[11px] uppercase tracking-[.14em] font-semibold text-ink-500 mt-6 mb-3 first:mt-0">{children}</h4>; }

function IdentityCheckpoint({lane,storedIdentity}:{lane?:LaneViewState;storedIdentity?:MerchantIdentity|null}) {
  const identity=(lane?.data as MerchantIdentity|undefined)||storedIdentity||undefined;
  const status=storedIdentity&&lane?.status==="queued"?"complete":lane?.status||"queued";
...
function RiskReport({result}:{result:MerchantRiskResult}) {
  ...
  const bannerText=needsReview?"Supported signals require review":insufficient?"Identity or evidence coverage is incomplete":"No material indicators found in reviewed sources";
  const labels=[
  ...
  return <article className="glass rounded-2xl p-5 md:p-8"><div className="max-w-4xl mx-auto">
    <header className="pb-6"><div className={`${bannerColor} text-xs font-semibold uppercase tracking-wider flex gap-2 items-center`}><BannerIcon className="w-4 h-4"/>{bannerText}</div><h2 className="font-display text-2xl md:text-3xl font-semibold text-ink-100 mt-3">{identity?.canonical_name||result.merchant_name}</h2></header>
    <dl className="border-y border-ink-700/40 divide-y divide-ink-700/30">{labels.map(([label,value])=><div key={label} className="py-3 flex flex-wrap justify-between gap-2 text-sm"><dt className="text-ink-500">{label}</dt><dd className="font-medium text-ink-100 capitalize">{value}</dd></div>)}</dl>

    {identity&&<ReportSection number="01" title="Resolved identity"><div className="space-y-2 text-sm"><p><span className="text-ink-500">Domain: </span><span className="text-ink-200">{identity.canonical_domain||"Unverified"}</span></p><p><span className="text-ink-500">Country: </span><span className="text-ink-200">{identity.country||"Unverified"}</span></p></div>{identity.business_description&&<p className="text-sm text-ink-300 leading-6 mt-4">{identity.business_description}</p>}<Subheading>Match basis</Subheading><List items={identity.match_basis}/>{[...new Set([...result.identity_ambiguities,...identity.ambiguities])].length>0&&<><Subheading>Identity caveats</Subheading><List items={[...new Set([...result.identity_ambiguities,...identity.ambiguities])]}/></>}</ReportSection>}

    {result.category_fit&&<ReportSection number="02" title={`Category fit · ${result.category_fit.category_fit.replace(/_/g," ")}`}><p className="text-sm text-ink-300 leading-6">{result.category_fit.explanation}</p><Subheading>Observed offerings</Subheading><EvidenceRows items={result.category_fit.observed_offerings}/></ReportSection>}

    {result.restricted_products&&<ReportSection number="03" title="Restricted products and services"><EvidenceRows items={result.restricted_products.indicators}/><Subheading>Products reviewed</Subheading><List items={result.restricted_products.products_reviewed} empty="No specific restricted-product indicators were returned."/><Subheading>Coverage gaps</Subheading><List items={result.restricted_products.evidence_gaps}/></ReportSection>}

    {result.reputation&&<ReportSection number="04" title="Reputation and business practices"><EvidenceRows items={result.reputation.indicators}/><Subheading>Observed themes</Subheading><List items={result.reputation.themes} empty="No material themes were returned."/><Subheading>Coverage gaps</Subheading><List items={result.reputation.evidence_gaps}/></ReportSection>}

    {result.legal_regulatory&&<ReportSection number="05" title="Legal and regulatory context"><EvidenceRows items={result.legal_regulatory.indicators}/><Subheading>Jurisdictions checked</Subheading><List items={result.legal_regulatory.jurisdictions_checked}/><Subheading>Coverage gaps</Subheading><List items={result.legal_regulatory.evidence_gaps}/></ReportSection>}
...
export default function MerchantRiskPage() {
  ...
  const identityReady = identityLane?.status === "complete" || Boolean(workflow.result?.resolved_identity);
  const researchLanes = workflow.lanes
    .filter((lane) => lane.id !== "identity")
    .map((lane) => lane.status === "queued" ? { ...lane, message: identityReady ? "Ready for resolved identity" : "Waiting for identity handoff" } : lane);
