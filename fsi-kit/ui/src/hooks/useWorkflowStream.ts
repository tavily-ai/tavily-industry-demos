...
const mergeSources = (left: ResearchSource[], right: ResearchSource[] = []) => {
  ...
  right.forEach((source) => source?.url && merged.set(sourceKey(source), { ...merged.get(source.url), ...source }));
  return [...merged.values()];
};

export function useWorkflowStream<TResult>(path: string, initialLanes: WorkflowLaneDefinition[]) {
  const makeLanes = useCallback(() => initialLanes.map((lane) => ({ ...lane, status: "queued" as const, queries: [], sources: [] })), [initialLanes]);
  ...
  const updateLane = useCallback((laneId: string, update: (lane: LaneViewState) => LaneViewState) => {
    setLanes((current) => {
      const found = current.some((lane) => lane.id === laneId);
      const next = found ? current : [...current, { id: laneId, label: laneId.replace(/_/g, " "), status: "queued", queries: [], sources: [] } as LaneViewState];
      return next.map((lane) => lane.id === laneId ? update(lane) : lane);
    });
  }, []);

  const handleEvent = useCallback((event: NormalizedWorkflowEvent) => {
    if (event.run_id) setRunId(event.run_id);
    switch (event.type) {
      case "start":
        if (event.lanes?.length) setLanes(event.lanes.map((lane) => ({ ...lane, status: "queued", queries: [], sources: [] })));
        break;
      case "progress":
        updateLane(event.lane_id, (lane) => ({ ...lane, status: "running", phase: event.phase, message: event.message, queries: event.queries ?? lane.queries }));
        break;
      case "sources_found":
        setSources((current) => mergeSources(current, event.sources));
        updateLane(event.lane_id, (lane) => ({ ...lane, status: "running", sources: mergeSources(lane.sources, event.sources) }));
        break;
      case "lane_complete":
      case "category_complete": { // temporary alias accepted during migration
      ...
      case "error": {
        ...
        if (laneId) {
          ...
          updateLane(laneId, (lane) => ({ ...lane, status: "error", error: event.message, message: event.message }));
        } else {
  ...
  const run = useCallback(async (body: Record<string, unknown>) => {
    ...
    setLanes(makeLanes()); setResult(null); setSources([]); setError(null); setRunId(null); setElapsedS(null); setStatus("running");
    try {
  ...
  const stop = useCallback(() => {
    abortRef.current?.abort();
    setStatus("cancelled");
    setLanes((current) => current.map((lane) => lane.status === "running" ? { ...lane, status: "queued", message: "Stopped before completion" } : lane));
  }, []);

  const hydrate = useCallback((stored: TResult, storedSources: ResearchSource[] = [], storedRunId?: string) => {
    abortRef.current?.abort(); setResult(stored); setSources(storedSources); setRunId(storedRunId ?? null); setError(null); setStatus("complete"); setLanes(makeLanes());
  }, [makeLanes]);
