  ...
  return (
    <details className="glass rounded-2xl group" aria-label={title}>
      <summary className="list-none cursor-pointer px-5 py-4 flex items-center gap-3 select-none">
        <Globe2 className="w-4 h-4 text-tavily-green" />
        ...
        <span className="ml-auto text-[11px] text-ink-500 hidden group-open:inline">
          Hide sources
        </span>
      ...
      <div className="border-t border-white/55 px-5 pb-5 pt-2 grid md:grid-cols-2 gap-x-6">
        {sources.map((source, index) => {
          ...
          return (
            ...
            >
              <span className="w-6 h-6 shrink-0 rounded-full bg-tavily-green/10 text-[11px] font-semibold text-tavily-green flex items-center justify-center">
                {index + 1}
              </span>
              <span className="min-w-0 flex-1">
                <span className="text-sm font-medium text-ink-100 group-hover/source:text-tavily-green block leading-5">
                  {source.title || host}
                </span>
                {!compact && (source.snippet || source.content) && (
                  <span className="text-xs text-ink-400 mt-1 line-clamp-2 block">
