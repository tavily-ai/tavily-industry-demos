      ...
      <div className="border-t border-white/55 px-5 pb-5 pt-2 grid md:grid-cols-2 gap-x-6">
        {sources.map((source, index) => {
          ...
          return (
            ...
            >
              <span className="w-6 h-6 shrink-0 rounded-full bg-tavily-orange/10 text-[11px] font-semibold text-tavily-orange flex items-center justify-center">
                {index + 1}
              </span>
              <span className="min-w-0 flex-1">
                <span className="text-sm font-medium text-ink-100 group-hover/source:text-tavily-orange block leading-5">
                  {source.title || host}
                </span>
