...
async def resolved_identity_stream(request, audit):
    ...
    yield {
        ...
        "detail": '"Fashion Nova" official website',
    }
    yield {
        "type": "log",
        "entry": {
            ...
            "hits": [
                {
                    "title": "Fashion Nova | Official Site",
                    "url": "https://www.fashionnova.com/",
                },
                {
                    "title": "FTC Takes Action Against Fashion Nova",
                    "url": "https://www.ftc.gov/news-events/fashion-nova",
                },
            ],
        },
    }
    yield {
        "type": "stage",
        "stage": "Reading articles",
        "detail": "https://www.fashionnova.com/",
    }
    yield {
        "type": "result",
        "data": {
            ...
            "confidence": "high",
            "match_basis": [
                "Official site and FTC source agree on the merchant identity."
            ],
            "ambiguities": [],
...
class MerchantIdentityTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_workflow_hands_agent_identity_to_four_research_lanes(self):
        ...
        async def fake_research(query, schema):
            ...
            if "Compare products" in query:
                ...
                content = {
                    "indicators": [],
                    "jurisdictions_checked": ["United States"],
                    "evidence_gaps": [],
                }
            yield {"content": content}

        with (
            mock.patch("backend.modules.merchant_risk.workflow.save_run"),
            mock.patch(
                "backend.modules.merchant_risk.workflow.AuditTrail.flush",
                return_value="/tmp/identity.jsonl",
            ),
        ):
            records = []
            async for raw in stream_merchant_risk(
                request,
                research_stream=fake_research,
                identity_stream=resolved_identity_stream,
            ):
                records.append(json.loads(raw.removeprefix("data: ").strip()))

        identity_complete = next(
            index
            for index, item in enumerate(records)
            if item["type"] == "lane_complete" and item["lane_id"] == "identity"
        )
        first_risk_progress = next(
            index
            for index, item in enumerate(records)
            if item["type"] == "progress" and item["lane_id"] != "identity"
        )
        self.assertLess(identity_complete, first_risk_progress)
        self.assertEqual(len(research_queries), 4)
        self.assertTrue(
            all(
                '"canonical_domain": "fashionnova.com"' in query
        ...
        identity_sources = next(
            item
            for item in records
            if item["type"] == "sources_found" and item["lane_id"] == "identity"
        )
    ...
    async def test_unresolved_identity_skips_research_lanes(self):
        ...
        request = MerchantRiskRequest(
            merchant_name="Ambiguous Name", category_code="5999"
        )
        ...
        with (
            ...
            async for raw in stream_merchant_risk(
                request,
                research_stream=fake_research,
                identity_stream=unresolved_stream,
            ):
                records.append(json.loads(raw.removeprefix("data: ").strip()))
