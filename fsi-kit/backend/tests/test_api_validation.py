...
class RequestValidationTests(unittest.TestCase):
    ...
    def test_investment_topic_required_and_nonblank_length(self):
        self.assertEqual(self.client.post("/api/investment-research/stream", json={}).status_code, 422)
        self.assertEqual(self.client.post("/api/investment-research/stream", json={"topic": "x"}).status_code, 422)

    def test_merchant_name_and_category_required(self):
        self.assertEqual(self.client.post("/api/merchant-risk/stream", json={"merchant_name": "Acme"}).status_code, 422)
        response = self.client.post("/api/merchant-risk/stream", json={"category_code": "5999"})
