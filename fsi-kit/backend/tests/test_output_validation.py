...
from backend.modules.investment_research.schemas import (
    EconomicDataResearch,
    EvidencePoint,
)
from backend.modules.merchant_risk.schemas import (
    CategoryFitResearch,
    MerchantEvidence,
    MerchantIdentity,
)
from backend.modules.merchant_risk.workflow import (
    _normalize_category_fit,
...
class ResearchOutputValidationTests(unittest.TestCase):
    ...
    def test_merchant_lanes_accept_partial_and_non_enum_values(self):
        ...
        category = CategoryFitResearch.model_validate(
            {"category_fit": "does not match"}
        )
