...
async def watchlist_stream(request: WatchlistRequest):
    ...
    return StreamingResponse(run_watchlist(max_clients=request.max_clients), media_type="text/event-stream", headers=SSE_HEADERS)
...
async def investigate_stream(request: InvestigateRequest):
    ...
    async def event_stream():
        ...
        async for event in stream_agent_run(
            ...
            task=investigation_task(request.query, request.flag_context), audit=audit,
        ):
        ...
        save_run(case_id, kind="investigation", query=request.query, elapsed_s=elapsed,
                 payload={"case_file": case_file, "flag_context": request.flag_context, "audit_log": str(log_path)})
