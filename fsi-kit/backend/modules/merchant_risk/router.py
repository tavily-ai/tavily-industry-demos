...
async def merchant_risk_stream(request: MerchantRiskRequest):
    missing = [name for name in ("TAVILY_API_KEY", "NEBIUS_API_KEY") if not (os.getenv(name) or "").strip()]
    if missing:
        raise HTTPException(status_code=503, detail=f"Missing server credential(s): {', '.join(missing)}")
