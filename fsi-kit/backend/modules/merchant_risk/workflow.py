...
def _normalize_category_fit(value: Any) -> str:
    ...
    aliases = {
        "matches": "consistent",
        "match": "consistent",
        "partially_consistent": "mixed",
        "partial": "mixed",
        "does_not_match": "inconsistent",
...
def _final_context(
    ...
    category_fit = (
        dict(results["category_fit"]) if results.get("category_fit") else None
    )
    if category_fit is not None:
        category_fit["category_fit"] = _normalize_category_fit(
            category_fit.get("category_fit")
        )
    ...
    if confidence in ("low", "unresolved"):
        gaps.append(
            "Merchant identity is not confidently resolved; findings may be incomplete or misattributed."
        )
    unique_sources = normalize_sources(sources)
    completed_risk_lanes = sum(
        lane in results
        for lane in (
            "category_fit",
            "restricted_products",
...
IdentityStream = Callable[
    [MerchantRiskRequest, AuditTrail], AsyncIterator[dict[str, Any]]
]


async def _default_identity_stream(
    request: MerchantRiskRequest, audit: AuditTrail
...
async def stream_merchant_risk(
    ...
    *,
    ...
    yield sse(
        {
            ...
            "run_id": run_id,
            "workflow": WORKFLOW,
            "lanes": [{"id": i, "label": label} for i, label in LANE_META],
        }
    )
    results: dict[str, Any] = {}
    errors: dict[str, str] = {}
    ...
    audit.record(
        "identity_start",
        merchant_name=request.merchant_name,
        submitted_domain=request.domain,
    )
    ...
    async for agent_event in run_identity(request, audit):
        ...
        if event_type == "stage":
            ...
            if found:
                ...
                new_sources = [
                    item for item in sources if item.get("url") not in before
                ]
                if new_sources:
                    yield sse(
                        {
            ...
            identity = MerchantIdentity.model_validate(
                agent_event.get("data") or {}
    ...
    identity_unresolved = (
        not identity
        or not identity.get("canonical_domain")
        or identity.get("confidence") == "unresolved"
    )
    if identity_unresolved:
        block_reason = (
            "Blocked because the identity agent did not establish a merchant domain"
        )
        ...
        async for event in orchestrate_lanes(
            run_id=run_id,
            workflow=WORKFLOW,
            lanes=risk_lanes(request, identity),
    ...
    save_run(
        ...
        elapsed_s=elapsed,
        counts={
            "lanes_complete": len(results),
            "lanes_error": len(errors),
            "lanes_skipped": len(skips),
        },
        payload={
            "request": request.model_dump(mode="json"),
            "result": final_data,
            "sources": sources,
            "identity_audit_log": str(audit_path),
        },
    )
    yield sse(
        {
