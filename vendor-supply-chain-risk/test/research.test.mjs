...
const fakeSearch = async (url, options) => {
  const domains = options ? JSON.parse(options.body).include_domains : undefined;
  const domain = domains?.[0] || "example.org";
...
test("all example inputs produce focused advanced requests for each lane", () => {
  for (const example of config.examples) {
    ...
    for (const lane of plan) {
      ...
      if (lane.id === "sanctions") {
        assert.ok(lane.request.include_domains.length > 0);
        assert.ok(lane.request.include_domains.every((domain) => !domain.includes("/")));
        assert.ok(!("time_range" in lane.request));
      } else {
        assert.ok(!("include_domains" in lane.request));
        assert.equal(lane.request.time_range, "month");
      }
    }
  }
});
...
test("searches execute concurrently and emit real completions", async () => {
  ...
  assert.ok(report.sections[0].sources[0].favicon.endsWith("/favicon.ico"));
...
test("partial upstream failures keep successful sources and sanitize API errors", async () => {
  ...
  const report = await research(input, {
    ...
    fetchImpl: async (url, options) =>
      ++count === 2 ? new Response("test-secret", { status: 429 }) : fakeSearch(url, options),
  });
        ...
        results: [
          {
            ...
            title: "Unrelated",
            content: "Outside the requested domains.",
          },
        ],
      }),
  });
  for (const section of report.sections.filter((section) => section.request.include_domains)) {
    assert.equal(section.sources.length, 0);
    assert.equal(section.summary, null);
  }
  const openWeb = report.sections.filter((section) => !section.request.include_domains);
  assert.ok(openWeb.length);
  assert.ok(openWeb.every((section) => section.sources.length === 1));
});
