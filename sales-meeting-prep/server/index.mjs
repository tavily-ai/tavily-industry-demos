...
export function createApp({
  ...
  return createServer(async (req, res) => {
    ...
    if (path === "/api/health" && req.method === "GET")
      return json(res, 200, { ok: true, hasServerKey: Boolean(serverKey) });
    if (path === "/api/research" && req.method === "POST") {
      ...
      try {
        ...
        if (!serverKey)
          return json(res, 401, {
            error: "Set TAVILY_API_KEY in .env to start a live search, or open the saved example.",
          });
        const now = Date.now();
        for (const [ip, time] of recent) if (now - time > 60000) recent.delete(ip);
        const ip = req.socket.remoteAddress;
        if (active >= 4 || now - (recent.get(ip) || 0) < 2000)
        ...
        if (!res.headersSent)
          ...
          res.write(
            `data: ${JSON.stringify({ type: "error", message: "Research stopped unexpectedly. Please retry." })}\n\n`,
          );
          res.end();
        }
      } finally {
