...
export function canonicalUrl(value) {
  try {
    ...
    if (/(^|\.)amazon\.[a-z.]+$/.test(url.hostname) && asin) {
...
export function diverseSources(items, { limit = 8, perPublisher = 2 } = {}) {
  ...
  for (const item of items || []) {
    ...
    if (
      !previous ||
      (Number(candidate.score) || 0) > (Number(previous.score) || 0) ||
      String(candidate.content).length > String(previous.content).length
    )
  ...
  for (let round = 0; round < perPublisher && result.length < limit; round++) {
    for (const group of groups.values())
      if (group[round] && result.length < limit) result.push(group[round]);
  }
  return result;
}
