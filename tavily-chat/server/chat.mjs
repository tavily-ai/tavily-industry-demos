...
export function validateInput(body) {
  if (!body || typeof body.message !== "string" || !body.message.trim())
    throw new InputError("Type a question to start chatting.");
  if (body.message.length > config.maxMessageLength)
    throw new InputError(`Keep your question under ${config.maxMessageLength} characters.`);
  const history = body.history ?? [];
  if (!Array.isArray(history) || history.length > config.maxHistory)
    throw new InputError("Send at most six previous messages.");
  for (const item of history) {
    if (
      ...
      throw new InputError("Invalid conversation history.");
  }
  if (history.length % 2 || history.some((item, i) => item.role !== (i % 2 ? "assistant" : "user")))
    throw new InputError("Conversation history must contain complete question and answer pairs.");
  return {
    ...
    tools: [
      {
        type: "function",
        name: "tavily_search",
        description: "Search the public web for evidence for the latest question.",
        strict: true,
        parameters: {
          type: "object",
          properties: {
            query: {
              type: "string",
              description: "Focused query for primary evidence under 400 characters.",
            },
            corroborating_query: {
              type: "string",
              description: "Complementary query for independent evidence, under 400 characters.",
            },
            primary_domains: {
              type: "array",
              items: { type: "string" },
              ...
              maxItems: 5,
              description:
                "Known authoritative publisher domains relevant to this question, without paths or protocols.",
            },
          },
          required: ["query", "corroborating_query", "primary_domains"],
...
export function publicError(error) {
  ...
  if ([401, 403].includes(error.status))
    return (
      provider +
      " rejected the server API key. Check " +
      (provider === "Tavily" ? "TAVILY_API_KEY" : "OPENAI_API_KEY") +
      " and retry."
    );
  if ([402, 432, 433].includes(error.status))
    return provider + " has reached its usage limit. Check your account.";
  if (error.status === 429)
    return (
      provider +
      " is receiving too many requests or has reached its quota. Check your account and try again."
    );
  if (error.name === "TimeoutError")
    return "The answer took too long. Try a more focused question.";
...
export async function streamAnswer(response, emit = () => {}) {
  ...
  for await (const frame of readSSE(response)) {
    ...
    if (chunk.error || ["error", "response.failed", "response.incomplete"].includes(chunk.type))
    ...
    if (chunk.type === "response.completed") {
      complete = chunk.response?.status === "completed";
      break;
    }
  }
  ...
  try {
    ...
    async function request(url, key, body) {
      ...
      if (!response.ok) {
        ...
        throw Object.assign(new Error("Provider request failed"), { status: response.status });
      }
      return response;
    }
    emit({
      ...
      message: "Focusing your question using the conversation",
      queries: [],
    });
    const plan = buildRequest(input);
    const planned = await (
      await request("https://api.openai.com/v1/responses", openaiApiKey, plan)
    ).json();
    const calls = planned.output?.filter((item) => item.type === "function_call") || [];
    if (
      planned.status !== "completed" ||
      calls.length !== 1 ||
      calls[0].name !== "tavily_search" ||
      !calls[0].call_id
    )
      throw new Error("Invalid search plan");
    const call = calls[0],
      args = JSON.parse(call.arguments);
    if (typeof args.query !== "string" || !args.query.trim() || args.query.length > 400)
      throw new Error("Invalid search query");
    provider = "Tavily";
    if (
      args.corroborating_query != null &&
      (typeof args.corroborating_query !== "string" ||
        !args.corroborating_query.trim() ||
        args.corroborating_query.length > 400)
    )
      throw new Error("Invalid corroborating query");
    const primaryDomains = args.primary_domains || [];
    if (
      !Array.isArray(primaryDomains) ||
      primaryDomains.length > 5 ||
      primaryDomains.some(
        (domain) => typeof domain !== "string" || !/^(?:[a-z0-9-]+\.)+[a-z]{2,}$/i.test(domain),
      )
    )
      throw new Error("Invalid primary domains");
    const queries = [
      ...new Set([
        args.query,
        args.corroborating_query || args.query.slice(0, 360) + " independent analysis evidence",
      ]),
    ];
    emit({
      type: "activity",
      stage: "Searching the web",
      message: "Comparing primary and independent sources",
      queries,
    });
    const outcomes = await Promise.allSettled(
      queries.map(async (query, index) => {
        const search = await (
          await request("https://api.tavily.com/search", apiKey, {
            query,
            search_depth: "advanced",
            include_domains: index === 0 ? primaryDomains : [],
            ...(index === 0 && primaryDomains.length ? { include_domains_mode: "filter" } : {}),
            topic: "general",
            max_results: 10,
            chunks_per_source: 3,
    ...
    if (successful.length < queries.length)
      emit({
        type: "activity",
        stage: "Using available sources",
        message: "One search was unavailable; using the evidence retrieved.",
        queries: [],
      });
    const items = diverseSources(
      successful.flatMap((item) => item.value),
      { limit: 8, perPublisher: 2 },
    );
    const sources = normalizeSources(items);
    const evidence = sources.map((source) => ({
      ...source,
      content: String(items[source.id - 1].content || "").slice(0, 12000),
    }));
    ...
    emit({
      ...
      message: "Answering from the search results",
      queries: [],
    });
    const response = await request("https://api.openai.com/v1/responses", openaiApiKey, {
      ...
      input: [
        ...
        ...planned.output,
        {
          ...
          output: JSON.stringify({ sources: evidence }),
        },
      ],
    });
