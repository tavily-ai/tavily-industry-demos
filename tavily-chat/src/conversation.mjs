export function conversationHistory(messages) {
  ...
  for (let i = 0; i < messages.length - 1; i++) {
    ...
    if (user.role === "user" && assistant.role === "assistant" && assistant.status === "complete") {
      pairs.push([
        { role: "user", content: user.content.slice(0, 2000) },
        { role: "assistant", content: assistant.content.slice(0, 8000) },
      ]);
    }
  }
...
export function remarkSourceList({ sources = [] } = {}) {
  ...
  return (tree) => {
    ...
    for (let index = 0; index < nodes.length; index++) {
      ...
      if (
        node.type !== "heading" ||
        !/^(sources|references|bibliography)$/i.test(
          (node.children || [])
      ...
      if (!tail.length || !tail.every((child) => ["list", "thematicBreak"].includes(child.type)))
        continue;
      const links = [];
      const collect = (child) => {
        if (child.type === "link") links.push(safeLink(child.url));
        for (const item of child.children || []) collect(item);
      };
      tail.forEach(collect);
      if (links.length && links.every((url) => known.has(url))) {
        tree.children = nodes.slice(
          0,
          nodes[index - 1]?.type === "thematicBreak" ? index - 1 : index,
        );
        break;
      }
    }
      ...
      node.children = node.children.flatMap((child) => {
        ...
        for (const match of child.value.matchAll(/\[(\d+(?:\s*,\s*\d+)*)\]/g)) {
          ...
          if (match.index > offset)
            nodes.push({ type: "text", value: child.value.slice(offset, match.index) });
          for (const id of numbers)
            nodes.push({
              type: "link",
              url: `#source-${prefix}-${id}`,
