...
const CrawlResults: React.FC<CrawlResultsProps> = ({
  ...
  // Use URLs from new format if available, otherwise use crawlResults
  const displayUrls = urls || (crawlResults ? crawlResults.map(item => item.url) : []);
  const displayFavicons = favicons || (crawlResults ? crawlResults.map(item => item.favicon) : []);
        ...
        <div className="w-[60%] space-y-2">
          {crawlBaseUrl && (
            <div className="text-sm font-medium text-gray-700 mb-2">
              Crawled from {crawlBaseUrl} ({displayUrls.length} page{displayUrls.length !== 1 ? "s" : ""})
            </div>
          )}
