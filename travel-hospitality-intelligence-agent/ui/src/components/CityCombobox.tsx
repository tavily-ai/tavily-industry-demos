...
const CityCombobox = ({ value, onChange, className }: CityComboboxProps) => {
  ...
  const options = useMemo(() => {
    ...
    return CITIES.filter((city) => city.toLocaleLowerCase().includes(query))
  ...
  return (
    <div className="city-combobox" ref={containerRef}>
      ...
      />
      <button type="button" className="city-combobox-toggle" onClick={() => setIsOpen((open) => !open)} aria-label="Show city list" tabIndex={-1}>
        <ChevronDown size={18} />
      </button>
      {isOpen && (
        <ul id="city-options" className="city-options" role="listbox" aria-label="City suggestions">
          {options.length ? options.map((city, index) => (
            <li key={city} role="option" aria-selected={index === activeIndex}>
              <button type="button" className={index === activeIndex ? 'is-active' : ''} onMouseDown={(event) => event.preventDefault()} onClick={() => choose(city)}>{city}</button>
            </li>
          )) : <li className="city-options-empty">No matching city — keep typing to use a custom destination.</li>}
        </ul>
      )}
    </div>
  );
