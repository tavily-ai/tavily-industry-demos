...
const TravelFocusCombobox = ({
  value,
  onChange,
  className,
  id = "travelFocus",
  suggestions = TRAVEL_FOCUSES,
  label = "focus",
}: TravelFocusComboboxProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);
  const options = suggestions.filter((focus) =>
    focus.toLocaleLowerCase().includes(value.trim().toLocaleLowerCase()),
  );
  ...
  return (
    <div className="city-combobox" ref={containerRef}>
      ...
      {isOpen && (
        <ul
          id={`${id}-options`}
          className="city-options travel-focus-options"
          role="listbox"
          aria-label={`${label} suggestions`}
        >
          {options.map((focus) => (
            <li key={focus} role="option">
              <button
                type="button"
          ...
          {!options.length && (
            <li className="city-options-empty">
              No matching {label} — keep typing to add your own.
            </li>
          )}
        </ul>
      )}
